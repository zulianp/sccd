...
static inline void compute_face_vertex_tolerance_soa(
    const T codomain_tol, const T v0sx, const T v0sy, const T v0sz,
                              ...
                              sccd::max<T>(
                                  ...
                                  sccd::max<T>(
                                      ...
                                      sccd::max<T>(
                                          ...
                                          sccd::max<T>(
                                              sccd::abs<T>(ssa12 + ssa14 +
                                                           ssa17 - v1sz),
                                              sccd::max<T>(
                                                  sccd::abs<T>(ssa15 + ssa2 +
                                                               ssa4 - v1sx),
                                                  sccd::abs<T>(ssa16 + ssa7 +
                                                               ssa9 -
                                                               v1sy))))))))))));
  *tol1 = ssa18 /
          sccd::max<T>(
...
void compute_face_vertex_tolerance(
  ...
  for (int i = 0; i < 32; i++) {
    compute_face_vertex_tolerance_soa(
        codomain_tol[i], v0sx[i], v0sy[i], v0sz[i], v1sx[i], v1sy[i], v1sz[i],
        v2sx[i], v2sy[i], v2sz[i], v3sx[i], v3sy[i], v3sz[i], v0ex[i], v0ey[i],
        v0ez[i], v1ex[i], v1ey[i], v1ez[i], v2ex[i], v2ey[i], v2ez[i], v3ex[i],
        v3ey[i], v3ez[i], &tol0[i], &tol1[i], &tol2[i]);
  }
}
