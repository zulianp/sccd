...
imports it. Each file keeps its own name and stays runnable, because these are
standalone programs rather than functions — concatenating them into one `dead.py`
the way `spikes/src/dead.{hpp,cpp,cu}` collects dead C++ would produce a file
that cannot execute.

Run them against the virtual environment described in
[`python/README.md`](../../python/README.md); they need SymPy, and the plotting
one needs NumPy and Matplotlib.
