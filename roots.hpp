...
template <typename T>
static inline void vf_objective(
    const T sv[3],
    const T s1[3],
    const T s2[3],
    const T s3[3],
...
template <typename T>
static inline void vf_gradient(
    const T sv[3],
    const T s1[3],
    const T s2[3],
    const T s3[3],
    ...
    T out_g[3])
{
    ...
    out_g[0] = ssa13
            * (-ev[1] - ssa0 * (e1[1] - s1[1]) + sv[1] + u * (e2[1] - s2[1])
               + v * (e3[1] - s3[1]))
        + ssa19
            * (-ev[2] - ssa0 * (e1[2] - s1[2]) + sv[2] + u * (e2[2] - s2[2])
               + v * (e3[2] - s3[2]))
        + ssa7
            * (-ev[0] - ssa0 * (e1[0] - s1[0]) + sv[0] + u * (e2[0] - s2[0])
               + v * (e3[0] - s3[0]));
    ...
    T out_H[9])
{
    ...
    const T ssa2 = -ev[0] - ssa0 * ssa1 + sv[0] + u * (e2[0] - s2[0])
        + v * (e3[0] - s3[0]);
    const T ssa3 = e1[1] - s1[1];
    const T ssa4 = -ev[1] - ssa0 * ssa3 + sv[1] + u * (e2[1] - s2[1])
        + v * (e3[1] - s3[1]);
    const T ssa5 = e1[2] - s1[2];
    const T ssa6 = -ev[2] - ssa0 * ssa5 + sv[2] + u * (e2[2] - s2[2])
        + v * (e3[2] - s3[2]);
    const T ssa7 = t - 1;
    ...
    const T ssa32 = -ssa11 * ssa12 - ssa16 * ssa17 - ssa21 * ssa22
        - ssa25 * (-e2[0] + s2[0] + ssa1) - ssa28 * (-e2[1] + s2[1] + ssa3)
    ...
    const T ssa36 = -ssa12 * ssa33 - ssa17 * ssa34 - ssa22 * ssa35
        - ssa25 * (-e3[0] + s3[0] + ssa1) - ssa28 * (-e3[1] + s3[1] + ssa3)
...
template <typename T>
static inline void vf_objective_dir(
    const T sv[3],
    const T s1[3],
    const T s2[3],
    const T s3[3],
    ...
    T out_p[3])
{
    ...
    const T ssa21 = -ev[0] - ssa20 * ssa5 + sv[0] + u * (e2[0] - s2[0])
        + v * (e3[0] - s3[0]);
    const T ssa22 = e1[1] - s1[1];
    const T ssa23 = -ev[1] - ssa22 * ssa5 + sv[1] + u * (e2[1] - s2[1])
        + v * (e3[1] - s3[1]);
    const T ssa24 = e1[2] - s1[2];
    const T ssa25 = -ev[2] - ssa24 * ssa5 + sv[2] + u * (e2[2] - s2[2])
        + v * (e3[2] - s3[2]);
    const T ssa26 = 1.0
        / (2 * sccd::pow2<T>(ssa21) + 2 * sccd::pow2<T>(ssa23)
           + 2 * sccd::pow2<T>(ssa25));
    ...
    const T ssa36 = -ssa27 * ssa28 - ssa29 * ssa30 - ssa31 * ssa32
        - ssa33 * (-e2[0] + s2[0] + ssa20) - ssa34 * (-e2[1] + s2[1] + ssa22)
        - ssa35 * (-e2[2] + s2[2] + ssa24);
    const T ssa37 = 1.0
        / (-ssa26 * sccd::pow2<T>(ssa36) + 2 * sccd::pow2<T>(ssa27)
    ...
    const T ssa41 = -ssa28 * ssa38 - ssa30 * ssa39 - ssa32 * ssa40
        - ssa33 * (-e3[0] + s3[0] + ssa20) - ssa34 * (-e3[1] + s3[1] + ssa22)
        - ssa35 * (-e3[2] + s3[2] + ssa24);
    const T ssa42 = ssa26 * ssa36;
    const T ssa43 = 2 * ssa27 * ssa38 + 2 * ssa29 * ssa39 + 2 * ssa31 * ssa40
        - ssa41 * ssa42;
    const T ssa44 = ssa21 * ssa33 + ssa23 * ssa34 + ssa25 * ssa35;
    const T ssa45 =
        ssa27 * ssa33 + ssa29 * ssa34 + ssa31 * ssa35 + ssa42 * ssa44;
    const T ssa46 = (-ssa26 * ssa41 * ssa44 - ssa33 * ssa38 - ssa34 * ssa39
                     - ssa35 * ssa40 + ssa37 * ssa43 * ssa45)
        / (-ssa26 * sccd::pow2<T>(ssa41) - ssa37 * sccd::pow2<T>(ssa43)
           + 2 * sccd::pow2<T>(ssa38) + 2 * sccd::pow2<T>(ssa39)
           + 2 * sccd::pow2<T>(ssa40));
    ...
    T out_H[9])
{
    ...
    const T ssa24 = -ev[1] - ssa23 * ssa5 + sv[1] + u * (e2[1] - s2[1])
        + v * (e3[1] - s3[1]);
    const T ssa25 = 2 * ssa13;
    const T ssa26 = e1[2] - s1[2];
    const T ssa27 = -ev[2] - ssa26 * ssa5 + sv[2] + u * (e2[2] - s2[2])
        + v * (e3[2] - s3[2]);
    ...
    const T ssa38 = -ssa22 * (-e2[0] + s2[0] + ssa20)
        - ssa25 * (-e2[1] + s2[1] + ssa23) - ssa28 * (-e2[2] + s2[2] + ssa26)
        - ssa29 * ssa35 - ssa30 * ssa36 - ssa31 * ssa37;
    const T ssa39 = -ssa22 * (-e3[0] + s3[0] + ssa20)
        - ssa25 * (-e3[1] + s3[1] + ssa23) - ssa28 * (-e3[2] + s3[2] + ssa26)
        - ssa32 * ssa35 - ssa33 * ssa36 - ssa34 * ssa37;
...
template <typename T>
static inline void ee_objective(
    const T sv[3],
    const T s1[3],
    const T s2[3],
    const T s3[3],
    ...
    T* out_f)
{
    ...
    *out_f = sccd::pow2<T>(
                 ssa2 * (e3[0] * t + ssa1) - ssa3 * (e1[0] * t - s1[0] * ssa0)
                 + u * (e2[0] * t - s2[0] * ssa0) - v * (e4[0] * t + ssa1))
        + sccd::pow2<T>(
                 ssa2 * (e3[1] * t + ssa4) - ssa3 * (e1[1] * t - s1[1] * ssa0)
                 + u * (e2[1] * t - s2[1] * ssa0) - v * (e4[1] * t + ssa4))
        + sccd::pow2<T>(
                 ssa2 * (e3[2] * t + ssa5) - ssa3 * (e1[2] * t - s1[2] * ssa0)
                 + u * (e2[2] * t - s2[2] * ssa0) - v * (e4[2] * t + ssa5));
}

template <typename T>
static inline void ee_gradient(
    const T sv[3],
    const T s1[3],
    const T s2[3],
    const T s3[3],
    ...
    T out_g[3])
{
    ...
    out_g[0] = ssa16
            * (ssa1 * (e3[1] + ssa10) - ssa2 * (e1[1] - s1[1])
               + u * (e2[1] - s2[1]) - v * (e4[1] + ssa10))
        + ssa23
            * (ssa1 * (e3[2] + ssa17) - ssa2 * (e1[2] - s1[2])
               + u * (e2[2] - s2[2]) - v * (e4[2] + ssa17))
        + ssa9
            * (ssa1 * (e3[0] + ssa0) - ssa2 * (e1[0] - s1[0])
    ...
    T out_H[9])
{
    ...
    const T ssa30 = -2 * ssa10 * ssa23 - 2 * ssa15 * ssa4 - 2 * ssa19 * ssa7
        - ssa25 * (-e2[0] + s2[0] + ssa3) - ssa27 * (-e2[1] + s2[1] + ssa6)
...
template <typename T>
static inline void ee_objective_dir(
    const T sv[3],
    const T s1[3],
    const T s2[3],
    const T s3[3],
    ...
    T out_p[3])
{
    ...
    const T ssa26 = -ssa25 * ssa5 + ssa4 * (e3[2] + ssa24) + u * (e2[2] - s2[2])
        - v * (e4[2] + ssa24);
    const T ssa27 = 1.0
        / (2 * sccd::pow2<T>(ssa20) + 2 * sccd::pow2<T>(ssa23)
           + 2 * sccd::pow2<T>(ssa26));
    ...
    const T ssa34 = -2 * ssa20 * ssa28 - 2 * ssa23 * ssa29 - 2 * ssa26 * ssa30
        - ssa31 * (-e2[0] + s2[0] + ssa19) - ssa32 * (-e2[1] + s2[1] + ssa22)
        - ssa33 * (-e2[2] + s2[2] + ssa25);
    const T ssa35 = 1.0
        / (-ssa27 * sccd::pow2<T>(ssa34) + 2 * sccd::pow2<T>(ssa28)
    ...
    const T ssa45 = ssa39 * (-ssa28 * ssa36 - ssa29 * ssa37 - ssa30 * ssa38)
        - ssa43 * ssa44;
    const T ssa46 = ssa20 * ssa31 + ssa23 * ssa32 + ssa26 * ssa33;
    const T ssa47 =
        ssa28 * ssa31 + ssa29 * ssa32 + ssa30 * ssa33 + ssa44 * ssa46;
    const T ssa48 = -ssa27 * ssa43 * ssa46 + ssa35 * ssa45 * ssa47
        + 2 * t * (ssa40 + ssa41 + ssa42);
    const T ssa49 = -1.0
        / (ssa27 * sccd::pow2<T>(ssa43) + ssa35 * sccd::pow2<T>(ssa45)
           - 2
