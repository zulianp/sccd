...
#ifdef TIGHT_INCLUSION_ORACLE
    bool barycentricTriangle3D(const ticcd::Vector3& A,
...
#ifdef TIGHT_INCLUSION_ORACLE
    template <typename T>
    bool find_root_oracle(const int max_iter,
                          const T atol,
                          const T sv[3],
                          const T s1[3],
                          const T s2[3],
...
#ifdef TEST_ORACLE
                if (find_root_oracle<double>(4000, 1e-6, sv, s1, s2, s3, ev, e1, e2, e3, t, u, v)) {
                    toi[i] = t;
...
#else
                if (find_root_dfs<double>(4000, 1e-10, sv, s1, s2, s3, ev, e1, e2, e3, t, u, v, stack)) 
