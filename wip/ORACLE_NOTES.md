# Oracle: recorded runs and notes
...
`src/narrowphase/sccd_narrowphase_tight.hpp`). A caller may still *store* geometry as float;
...
less than the certified bound in `sccd_numerical_error.hpp`. All three modes pad by
...
1. **The "no zero toi" guard is correct and is in place.** `sccd_rootfinder.hpp`
   (`accepted = accepted && (tt_min > 0)`, four sites), the vertex-quad finder,
   and three sites in `sccd_narrowphase_fast_vector.hpp` refuse to accept a box whose `t`
